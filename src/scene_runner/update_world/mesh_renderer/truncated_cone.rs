...
pub struct TruncatedCone {
    /// Base radius in the XZ plane.
    pub base_radius: f32,
    /// Tip radius in the XZ plane.
    pub tip_radius: f32,
...
impl Default for TruncatedCone {
    fn default() -> Self {
        Self {
            base_radius: 0.5,
            tip_radius: 0.25,
...
impl From<TruncatedCone> for Mesh {
    fn from(c: TruncatedCone) -> Self {
        ...
        for ring in 0..num_rings {
            ...
            let height_frac = ring as f32 / (num_rings - 1) as f32;
            let radius = c.base_radius * (1.0 - height_frac) + c.tip_radius * height_frac;
        ...
        let mut build_cap = |top: bool| {
            ...
            let (y, normal_y, winding, radius) = if top {
                (c.height / 2., 1., (1, 0), c.tip_radius)
            } else {
                (c.height / -2., -1., (0, 1), c.base_radius)
            };

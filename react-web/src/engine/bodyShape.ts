...
/** Items to keep when switching body shape: Unity unequips what the new shape can't render. */
export function fittingUrns(equipped: Array<{ urn: string; category: string; bodyShapes?: string[] }>, bodyShape: string): Set<string> {
  return new Set(equipped.filter((w) => isCompatible(w, bodyShape)).map((w) => w.urn))
}

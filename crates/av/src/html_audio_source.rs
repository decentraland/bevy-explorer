...
use web_sys::{AudioBuffer, AudioBufferSourceNode, AudioContext, AudioScheduledSourceNode, GainNode, StereoPannerNode, js_sys::Float32Array};
...
impl HtmlAudioContext {
    ...
    fn start(
        ...
        if let Some(offset) = offset {
            source_node.start_with_when_and_grain_offset(0.0, offset as f64).ok()?;
        } else {
...
impl AudioGraphHtmlElements {
    pub fn stop(&self, now: f64) {
        let _ = self.gain_node.gain().linear_ramp_to_value_at_time(0.0, now + 0.01);

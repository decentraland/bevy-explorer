...
use crate::{AVPlayer, AVPlayerSinks, AVSinks, AudioStream, InScene, VideoPlayer};
...
impl Plugin for AudioSinkPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_observer(change_audio_sink_volume::<AudioStream>);
        app.add_observer(change_audio_sink_volume::<VideoPlayer>);
    }
}
...
fn av_sinks_inserted<T: AVPlayer>(
    ...
    debug!("AVSink<{}> inserted to {}", type_name::<T>(), entity);
...
fn av_sinks_replaced<T: AVPlayer>(trigger: Trigger<OnReplace, AVSinks<T>>, mut commands: Commands) {
    let entity = trigger.target();
    debug!("AVSink<{}> replaced on {}", type_name::<T>(), entity);
...
#[expect(clippy::type_complexity)]
fn change_audio_sink_volume<T: AVPlayer>(
    ...
    if let Some(mut audio_spawned) = maybe_audio_spawned {
        if in_scene {
            audio_spawned
                .handle
                .set_volume((volume * audio_settings.scene()) as f64, Tween::default());
        } else {
            audio_spawned.handle.set_volume(0.0, Tween::default());
        }
    }

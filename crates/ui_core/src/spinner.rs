...
fn setup(
    ...
    let texture_atlas_layout = TextureAtlasLayout::from_grid(Vec2::new(34.0, 34.0), 8, 1, None, None);
...
impl DuiTemplate for DuiSpinnerTemplate {
    fn render(
        ...
        let layout = props.borrow::<Handle<TextureAtlasLayout>>("spinner-layout", ctx)?.unwrap().clone();
        let image = props.borrow::<Handle<Image>>("spinner-image", ctx)?.unwrap().clone();

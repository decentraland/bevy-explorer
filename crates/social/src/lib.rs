...
#[derive(Event)]
pub struct FriendshipEvent(pub friendship_event_response::Body);

enum FriendshipOutbound {
    FriendshipEvent(FriendshipEventPayload),
    ChatMessage(DirectChatMessage),
    HistoryRequest(Address, UnboundedSender<DirectChatMessage>),
}
...
impl SocialClientHandler {
    ...
    pub fn get_chat_history(
        &self,
        address: Address,
    ) -> Result<UnboundedReceiver<DirectChatMessage>, anyhow::Error> {
...
async fn social_socket_handler(
    ...
    if let Err(e) = rt.block_on(social_socket_handler_inner(
        ...
        println!("social socket handler error: {e}");
    } else {
        println!("k..");
    }
}
...
async fn social_socket_handler_inner(
    ...
    let f_matrix_history = async move {
        while let Some((address, sx)) = rx_history.recv().await {
            ...
            let alias = room_alias(address)?;
            match client.join_room_by_id_or_alias(alias.as_ref()?, &[]).await {
                Err(e) => {
                    warn!("failed to find room for address {address:#?}");
                    warn!("err: {e}");
                    continue;
                }
                Ok(room) => {
                        ...
                        for event in history.chunk {
                            if let Ok(AnyTimelineEvent::MessageLike(m)) = event.event.deserialize()
                            {
                                if m.event_type() == MessageLikeEventType::RoomMessage {
                                    ...
                                    let _ = sx.send(DirectChatMessage {
                                        partner: address,

...
use bevy_mod_billboard::{BillboardTextBundle, text::BillboardTextBounds};
use common::{util::TryInsertEx, sets::SceneSets};
use dcl::interface::ComponentPosition;
use dcl_component::{SceneComponentId, proto_components::sdk::components::PbTextShape};
...
impl Plugin for TextShapePlugin {
    fn build(&self, app: &mut App) {
        app.add_crdt_lww_component::<PbTextShape, TextShape>(SceneComponentId::TEXT_SHAPE, ComponentPosition::EntityOnly);
...
fn update_text_shapes(
    ...
    let old_parents = query.iter().map(|(e, ..)| e).chain(removed.iter()).collect::<HashSet<_>>();
        ...
        commands.entity(ent).try_insert(BillboardTextBundle {
            text: Text::from_section(
                ...
                TextStyle {
                    ...
                    color: text_shape.0.text_color.map(Into::into).unwrap_or(Color::WHITE),
                    font: TEXT_SHAPE_FONT.get().unwrap().clone(),
                },
            ),
            text_bounds: BillboardTextBounds { size: Vec2::new(text_shape.0.width.map(|w| w*100.0).unwrap_or(f32::MAX), text_shape.0.height.map(|h| h*100.0).unwrap_or(f32::MAX)) },

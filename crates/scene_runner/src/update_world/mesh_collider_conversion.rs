...
//  use std::collections::BTreeMap;
 ...
 use bevy::{render::mesh::Indices, utils::HashMap, core::cast_slice, prelude::{Vec3, warn}};
 use rapier3d::{prelude::{SharedShape, Isometry, Real, Point}, parry::transformation::ConvexHullError};
use vhacd_rs::VHACDWrapperParams;
 
 pub async fn calculate_mesh_collider(positions: Vec<[f32; 3]>, maybe_indices: Option<Indices>, size_hint: Vec3, dbg: bool, label: String) -> Result<SharedShape, ConvexHullError> {
    let start = std::time::Instant::now();
     if dbg {
         println!("positions [{}]: {positions:?}\nindices [{:?}]: {maybe_indices:?}", positions.len(), maybe_indices.as_ref().map(|i| i.len()));
     }
     ...
     let mut positions_vec3 = Vec::with_capacity(positions.len());
     {
         ...
         for pos in positions.iter() {
             let id = vertex_lookup.entry(cast_slice::<f32,u8>(pos)).or_insert_with(|| { 
    ...
    //  while let Some(key) = group_joins.keys().next().copied() {
        ...
        let matching_indices = indices;
         ...
         if dbg {
            //  println!("group from {key}: {target_groups:?}");
             println!("includes [{}]: {matching_indices:?}", matching_indices.len());
         }
     
         // calculate extents
         let (min, max) = matching_indices.iter().fold((Vec3::MAX, Vec3::MIN), |(cmin, cmax), ix| {
        ...
        //      ])
        //  }).collect();
         let positions_vhacd: Vec<_> = matching_indices.iter().map(|ix| {
             let pos = positions_vec3[*ix];
         ...
         let indices_parry: Vec<_> = (0..tris).map(|ix| { let ix = ix as u32; [ix*3, ix*3+1, ix*3+2] }).collect();
         ...
         // AsyncComputeTaskPool::get().spawn(async move {
             ...
             let max_hulls = (resolution_unclamped / 10000).max(10) * 0 + 100_000;
             let error = 0.005;
             let depth = 12;
             ...
             for hull in hulls {
                 ...
                 let points: Vec<_> = hull.points.into_iter().map(|f32s| Point::from(Point::from(f32s) - Point::from(mid))).collect();
                 // let indices: Vec<_> = hull.indices.chunks_exact(3).map(|chunk| chunk.try_into().unwrap()).collect();
                 ...
                 let iso = Isometry {
                     ...
                     translation: nalgebra::Translation { vector: (mid * -1.0).into() },
                 };
    ...
    //  }
     ...
     if dbg {
         println!("[{label}] done ! {group_count} groups, {} shapes, {duration:?}", shapes.len());
     }
     Ok(SharedShape::compound(shapes))
 }

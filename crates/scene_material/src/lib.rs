...
// outline / dithering / show-outside moved to a mesh-tag scheme upstream
...
pub struct SceneBound {
    ...
    /// editor only: render this material on top of everything (depth test always
    /// passes, no depth write). A pipeline-specialization key, not a shader
    /// uniform — set by scene_runner's editor gizmo overlay. Inert when false.
    pub depth_off: bool,
}

use bevy::{prelude::*, utils::{HashMap, hashbrown::HashSet}, gltf::Gltf};
use common::util::TaskExt;
use comms::profile::UserProfile;
use ipfs::{EntityDefinition, IpfsAssetServer, ActiveEntityTask};
    ...
    if let Some(result) = task.as_mut().and_then(|t| t.complete()) {
        match result {
            Ok(res) => {
                ...
                defs.loaded.extend(res.iter().map(|def| def.pointers.get(0).cloned().unwrap_or_default()));
...
fn fetch_emote_details(
    ...
    for def in defs.unprocessed.drain(..) {
        let Some(first_glb) = def.content.files().find(|f| f.to_lowercase().ends_with(".glb")) else {
            warn!("no glb found in emote content map");
            continue;
        };

        ipfas.ipfs().add_collection(def.id.clone(), def.content.clone(), None, def.metadata.as_ref().map(serde_json::Value::to_string));
        let gltf = ipfas.load_content_file(first_glb, &def.id).unwrap();
        loading_gltf.push((def, gltf));
    }

    *loading_gltf = loading_gltf.drain(..).flat_map(|(def, h_gltf)| {
        match ipfas.asset_server().load_state(&h_gltf) {
            bevy::asset::LoadState::Loading => Some((def, h_gltf)),
            bevy::asset::LoadState::Loaded => {
                let metadata: EmoteMeta = def.metadata.and_then(|m| serde_json::from_value(m).ok()).unwrap_or_default();
                let anims = &gltfs.get(h_gltf).unwrap().named_animations;
                let not_starting_pose_anims = anims.iter().filter(|a| a.0 != "Starting_Pose").collect::<Vec<_>>();
                if not_starting_pose_anims.len() != 1 {
                    warn!("{} anims has {} members ({:?})", def.id, anims.len(), anims.keys());
                }
                if let Some(anim) = not_starting_pose_anims.first() {
                    avatar_anims.0.insert(def.pointers.get(0).cloned().unwrap_or_default(), AvatarAnimation {
                        name: metadata.name,
                        description: metadata.description,
                        clip: anim.1.clone(),
                        thumbnail: ipfas.load_content_file(&metadata.thumbnail, &def.id).unwrap(),
                    });
                }
                None
            },
